...
def url(year,month):
    return 'http://peregrinossantiago.es/eng/post-peregrinacion/estadisticas/?anio='+str(year)+'&mes='+str(month)
...
for year in range(LAST['year']+1,NOW['year']+1):
  for month in range(LAST['month']+1,NOW['month']):
    row_id=row_id+1
...
scraperwiki.sqlite.execute('create view if not exists pilgram as select * from age join foreigners join medium join months join motivation join sex join spanish where age.id and age.id = foreigners.id and age.id = medium.id and age.id = months.id and age.id = motivation.id and age.id = sex.id and age.id = spanish.id;')
